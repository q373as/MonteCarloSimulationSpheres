...
int main() {
    ...
    for(int t = 0; t < cfg.tsteps; t++){
        inter = std::abs(voxel.SimulateDiffusionSteps(cfg.DiffSteps, t * cfg.dt)) * std::exp(-  cfg.R2 * t * cfg.dt);
        stat = std::abs(voxel.ComputeSignalStatic(t * cfg.dt)) * std::exp(-  cfg.R2 * t * cfg.dt);

        result =   std::exp(-cfg.eta * f(1/cfg.tc, t * cfg.dt)) * std::exp(-  cfg.R2 * t * cfg.dt);
        ...
        star.push_back(std::exp(- cfg.R2p * t * cfg.dt) * std::exp(-  cfg.R2 * t * cfg.dt));
    }
